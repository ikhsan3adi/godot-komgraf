...
public static class GraphicsUtils
{
  ...
  public static void PutPixelAllCartesian(
    Node2D targetNode,
    List<Vector2> dot,
    DrawStyle style = DrawStyle.DotDot,
    Godot.Color? color = null,
    int stripLength = 3,
    int gap = 0)
  {
    PutPixelAll(targetNode, dot.Select(p => ScreenUtils.ConvertToPixel(p.X, p.Y)).ToList(), style, color, stripLength, gap);
  }
}